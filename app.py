...
# 【重要】初回登録時のみ必要な「秘密の合言葉」
# これを知らない部外者は、社員番号が合っていても登録できません。
SECRET_AUTH_CODE = "feelist55"  # ←ここを社内周知の言葉に変更してください
...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        # まず社員番号を入力させる
        target_id = st.text_input("社員番号を入力してください(4桁)", max_chars=4, key="login_id")
        
        if target_id:
            master = load_data_cached("UserMaster")
            if master.empty:
                st.error("マスターデータがありません。管理者に連絡してください。")
                return

            user_row = master[master['emp_id'].astype(str) == str(target_id)]

            if user_row.empty:
                st.error("入力された社員番号は登録を許可されていません。")
            else:
                # ユーザーが存在する場合の処理
                ...
                if pd.isna(stored_hash) or stored_hash == "":
                    st.info("⚠️ 初回ログイン：パスワード設定が必要です。")
                    with st.form("init_reg_form"):
                        auth_code = st.text_input("秘密の合言葉", type="password", help="社内で共有されている言葉を入力")
                        ...
                        if st.form_submit_button("登録してログイン"):
                            if auth_code != SECRET_AUTH_CODE:
                                ...
                                # ハッシュ化して保存
                                new_hash = make_hash(new_pw)
                                m_db = conn.read(worksheet="UserMaster", ttl="0s")
                                idx = m_db[m_db['emp_id'].astype(str) == str(target_id)].index[0]
                                m_db.at[idx, 'password_hash'] = new_hash
                                # もしニックネームがなければ社員番号を仮入れ
                                if pd.isna(m_db.at[idx, 'nickname']):
                                    m_db.at[idx, 'nickname'] = str(target_id)
                                
                                conn.update(worksheet="UserMaster", data=m_db)
                                st.session_state.authenticated = True
                                st.session_state.current_user = str(target_id)
                                st.cache_data.clear()
    ...
    master = load_data_cached("UserMaster")
    user_info = master[master['emp_id'].astype(str) == current_emp_id].iloc[0]
    current_nickname = user_info['nickname'] if pd.notna(user_info['nickname']) else current_emp_id

    st.title("📊 Dopamine Tracker")
    with st.sidebar:
        st.write(f"Logged in: **{current_nickname}** ({current_emp_id})")
        if st.button("ログアウト"):
            st.session_state.authenticated = False
            st.rerun()

    all_records = load_data_cached("Records")
    user_records = all_records[all_records['real_name'].astype(str) == current_emp_id].copy() if not all_records.empty else pd.DataFrame()
    ...
    with tab1:
        ...
        def record_ui():
            ...
            if st.button("この内容で登録する", type="primary"):
                ...
                st.cache_data.clear(); st.success("登録しました！"); time.sleep(1); st.rerun()
        record_ui()
    ...
    with tab4:
        ...
        if st.button("設定を保存する"):
            m_db = conn.read(worksheet="UserMaster", ttl="0s")
            idx = m_db[m_db['emp_id'].astype(str) == current_emp_id].index[0]
