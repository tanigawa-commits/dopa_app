...
# カスタムCSS：顔文字を圧倒的に大きく（64px）し、ボタンの高さも確保
st.markdown("""
    <style>
    div.stButton > button {
        font-size: 64px !important; /* 前回の倍、標準の約4倍のサイズ */
        height: 180px !important;   /* 顔文字がはみ出さない高さ */
        border-radius: 20px !important;
        line-height: 1.2 !important;
        transition: transform 0.2s;
    }
    div.stButton > button:active {
        transform: scale(0.95); /* 押した感を演出 */
    }
    /* 項目名（テキスト）のサイズ調整 */
...
# 投資型：自己効力感を構築し、長期的なメンタルを向上させる活動
INVESTMENT_MASTER = {
